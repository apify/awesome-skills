...
| [`apify-app-store-intelligence`](skills/apify-app-store-intelligence/SKILL.md) | Pull structured Apple App Store data — app metadata, price, rating, version, developer, and reviews — and watch it for changes over time. Use when the user asks to look up an iOS app by App ID, bundle ID or name, compare a set of competitor apps, monitor a competitor's price or rating for changes, track when an app ships a new version, scrape App Store reviews, resolve a bundle ID to a full app record, check an app's rating across multiple country storefronts, build an ASO or app-market dataset, or set up a recurring App Store watch that reports only what moved since last run. | [Donny](https://github.com/donnywin85) |
